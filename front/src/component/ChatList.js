            ...
            <div className="flex items-center">
              ...
              <div className="flex-1">
                <div className="flex justify-between">
                  <span className="font-medium text-gray-800 dark:text-primary-light"> {item._id === currentUser ? `${item.userName} (You)` : item.userName} </span>
              ...
              if (messageDate.toDateString() === now.toDateString()) {
                return messageDate.toLocaleTimeString([], { hour: "numeric", minute: "2-digit", hour12: false });
                ...
                {item?.messages?.[0]?.content.fileType?.startsWith('image/') ? (
                  ...
                  {item.messages?.filter((message) => message.receiver === currentUser && message.status !== "read").length > 0 && (
                    <div className={`ml-2 w-5 h-5  text-center flex items-center justify-center text-xs  font-semibold rounded-full
                     ${selectedChat?._id === item._id ?'bg-white text-primary-dark' : "bg-primary  text-white"}`}>
