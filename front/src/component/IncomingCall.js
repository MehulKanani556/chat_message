...
const IncomingCall = memo(() => {
  ...
  const { allUsers, groups, user } = useSelector((state) => state.user);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-white/15 flex items-center justify-center z-[9999]">
      <div className="bg-black rounded-lg p-6 w-60 md:w-96  text-center">
        ...
        <div className="w-14 h-14 md:w-20 md:h-20 mx-auto mb-5 md:mb-10 rounded-full overflow-hidden">
          {(userData?.photo) &&
            (userData?.photo !== "null") ? (
            <img
            ...
            <div className="w-14 h-14 md:w-20 md:h-20 mx-auto mb-4 rounded-full overflow-hidden bg-gray-300 grid place-content-center">
        ...
        <div className="flex justify-center gap-8">
          ...
          <button
            ...
            className="w-[40%] h-8 md:h-10 bg-[#22C55E] text-white text-sm md:text-base rounded-md flex items-center justify-center hover:bg-[#22C55E85] animate-bounce"
          >
            Accept
          </button>
        </div>
