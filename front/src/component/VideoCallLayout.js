    ...
    >
      ...
      {!chatMessages && (
        <div className="p-2  w-full flex justify-center items-center space-x-3 md:space-x-4 dark:bg-[#1A1A1A] bg-black/10">
          ...
          {isReceiving && isHost ? (
            <>
              <button onClick={() => unregisterAsHost()}>Stop Hosting</button>
            </>
          ) : (
            <>
              <button onClick={() => registerAsHost()}>Start Hosting</button>
              <button onClick={() => requestControl(hostId)} disabled={!isHost}>
                Request Control
              </button>
            </>
          )}
